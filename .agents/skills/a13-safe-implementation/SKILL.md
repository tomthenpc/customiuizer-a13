...
description: Implement exactly one approved atomic slice in tomthenpc/customiuizer-a13 on devin/a13-rom-intelligence-audit.
...
```text
@skills:a13-safe-implementation <task-slice-path>
```

`$ARGUMENTS` must identify one filled A13 Task Slice. If it is empty, ambiguous, or names more than one task, stop before editing.

## Repository lock

```text
Repository: tomthenpc/customiuizer-a13
Authorized branch: devin/a13-rom-intelligence-audit
Platform: MIUI 14 / Android 13，HyperOS 1 / Android 13 仅 contract-guarded
```
