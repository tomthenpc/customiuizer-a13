...
| P1B-1 | `cd89c38f02834db5baf24e5ab08b345c187085fe` | `44b4c4c` | `afd3d15` | `tasks/completed/A13-PERF-P1B-1-STARTUP-LOAD-GATING.md` | completed | `QA_CONDITIONAL` | Launcher 生产路由已接入 `MainModule`，五个集成场景通过；SystemUI gate integrity 与完整 Release/QA 总门尚未完成 | 拆分 PackageReady / Application 门控并接入 `MainModule` 路由；新增集成测试覆盖 all-off/package-ready-only/application-only/both 场景；完成 SystemUI gate integrity 并等待 device 证据 |
...
- 排除规则：跳过以 `pref_key_launcher_` 开头且以 `_action` 结尾的键；合法 SystemUI action（如 `controls_backlong_action`）仍被识别。
- 当前 QA 状态：`QA_CONDITIONAL`。原因：SystemUI gate integrity 尚未完成；完整 Release 与真机证据尚未完成。
