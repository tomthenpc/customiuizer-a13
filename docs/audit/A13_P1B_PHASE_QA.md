...
| P1B-1 | `cd89c38f02834db5baf24e5ab08b345c187085fe` | `44b4c4c` | `d1c19d2` | `tasks/completed/A13-PERF-P1B-1-STARTUP-LOAD-GATING.md` | completed | `QA_CONDITIONAL` | R1-A 完成：`SystemUiInstaller` 已使用正向 SystemUI 域识别全局 action；新增 `SystemUiGateTest` 和 `MainModuleSystemUiRoutingTest` 覆盖域污染与合法路径；R1-B 结构对账与 mutation 验证尚未完成 | 拆分 PackageReady / Application 门控并接入 `MainModule` 路由；新增集成测试覆盖 all-off/package-ready-only/application-only/both 场景；完成 R1-B 结构对账、差异修复与 device 证据 |
