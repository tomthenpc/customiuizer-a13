# A13/A14 Product Feature Inventory (Phase D-R1 corrected)
...
## Method correction summary

R1 replaces key-counting with typed inventory nodes and evidence levels:
...
- A14 feature-spec discovery no longer relies on parity by same-key; unresolved parse constructs are surfaced as unknown discovery counters.

```text
UI_TOPOLOGY_NODE_COUNT_A14 = 712
UI_TOPOLOGY_NODE_COUNT_A13 = 728
UNKNOWN_DISCOVERY_COUNT = 0
```
...
Discrepancy outputs are now conservative (`CANDIDATE`/`INTERNAL`) unless semantically proven.
