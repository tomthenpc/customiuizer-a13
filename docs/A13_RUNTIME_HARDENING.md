...
| `Launcher` hooks | Layout/event driven | Zero for disabled feature groups | Folder `onLayout` reuses cached reflection metadata and mutates existing layout state without collections | Hook/controller state only; cold Settings option construction is outside Launcher runtime | Launcher layout, gesture and icon events | Remaining gesture/icon callbacks and HyperOS Launcher targets need audit |
