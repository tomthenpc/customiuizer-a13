...
| `Launcher` hooks | Layout/event driven | Zero for disabled feature groups | Folder `onLayout` reuses cached reflection metadata and mutates existing layout state without collections; floating-window package exclusions use direct equality | Hook/controller state only; cold Settings option construction is outside Launcher runtime | Launcher layout, gesture and icon events; ordinary ROM misses remain isolated and OOM propagates | HyperOS Launcher targets still need Contract/device-log evidence |
...
| High-frequency callback allocations | PARTIAL | Clock hour-token and network-speed suffix replacement use direct scans with zero allocation on miss; `ResourceHooks` has no hot `Executable.getName()` | Remaining `String.format`, `StringBuilder`, `ArrayList` and lambda use in confirmed hot callbacks need pass |
...
| Lazy process-wide infrastructure | VERIFIED_STATIC | `MainModule.getResHooks()` uses JVM holder initialization; `GlobalActions` main Handler uses synchronized lazy initialization | Processes with no resource replacement/action requiring delayed main work no longer allocate these objects at class load |
...
| Controls input-path hardening | VERIFIED_STATIC | `MediaPlayer.getAudioStreamType` is resolved once per install; power-double-tap reasons use direct equality; all Controls fallbacks rethrow OOM; `HookerClassHelper` unwraps OOM through bounded cause chains | Key/action semantics and exact MIUI targets are unchanged; device input timing remains log/device verification work |
| Notification callback allocations | VERIFIED_STATIC | Notification count limit uses a capped loop with early exit instead of `filter`/temporary list; read-only hooks use `getArg` and avoid argument-array materialization; all local Throwable fallbacks rethrow OOM | Count limit remains 24 and mutation hooks retain materialized arguments only where they rewrite them |
...
| `LockScreenAlbumArtController` decode | media art/parameter/size change | enabled + non-null art + attached owner + screen on | media clear, screen off, owner detach, size/source generation change or missing ROM capability | not applicable | cancels queued work/publication; owner detach clears the one-frame cache, processed static Bitmap and exact module-owned Drawable while retaining the source for reattach; private intermediate Bitmaps are recycled | lock-screen album art pref | one bounded worker queue; generation + interruption gate; tokenized main publication | static large-object lifecycle complete; legacy MIUI target bundle and peak RSS need device-log/device confirmation |
