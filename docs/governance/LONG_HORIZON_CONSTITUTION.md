# A13/A14 Common Long-Horizon Constitution v5
...
EvidenceCommit: ab15088739b6ee67ce508a948ae106b4b1eb8e61
...
本文件是 A13/A14 的长期工程宪章。产品专属 `GOAL.md` 高于本文件；本文件补充长期治理，不扩大产品 Android/ROM 边界。
...
A13/A14 不直接扩展到下一 Android 大版本。它们必须输出未来迁移资产：
...
- `ACTIVE_HARDENING`：架构修复、性能优化、兼容补强和安全迁移。
- `RELEASE_CANDIDATE`：机器门禁通过，等待真实设备、签名和最终回归。
- `STABLE`：有正式设备证据、RC 和已知限制。
- `LTS`：默认 feature freeze，以稳定、兼容、构建恢复和高优先级修复为主。
- `SECURITY_ONLY`：只处理 P0/P1、构建链失效、严重兼容、安全与供应链、设备 bootloop/fatal。
- `ARCHIVE_READY`：最终源码可构建、依赖工具链已归档说明、artifact/hash 可追溯、known limitations 完整、无未说明 secret、迁移资产已导出。
- `ARCHIVED`：只读历史状态。不得伪装为继续支持。
...
不建立联网遥测。必须提供本地、可脱敏的 diagnostic bundle：
...
要求：默认不采集个人数据、不上传、导出由用户主动触发、脱敏、diagnostics 不得成为热路径负担、schema version 固定。
...
每个 RC 记录：commit、schema versions、supported devices/ROM、known limitations、artifact hash、signing identity summary、rollback instructions、preference migration、device evidence。
...
机械数字由生成器维护。每次 qualifying commit 必须保持 code、tests、state、generated docs、current architecture、verification evidence 在同一事务。

## 13. 自治维护

Agent 可以持续发现和修复，但禁止 artificial churn。没有高价值变更时：
...
自动任务必须有：Evidence、Impact、Risk、Acceptance、Rollback、Verification。
