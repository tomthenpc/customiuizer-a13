...
> 参考 A14 `main` 正式版 `r14.13.5`（`4225d80e`）与 `devin/r14.13-kotlin-refactor` 最新 `321081d2`，
...
| 12 | ResourceHooks | `ResourceHooks.kt` 已优化：volatile copy-on-write、`SparseArray`、延迟 `chain.executable.name`、未命中短路径 | `ResourceHooks.java` 仍是 Java，已做单次 `get()` 优化，但仍存在 `ModuleHelper.findContext()` 热路径调用 | 需要按 A13 改写 | 高 | 移植 A14 的 copy-on-write / 延迟 executable name / 未命中短路径模式，保持 Java 实现或评估 Kotlin | `assembleRelease` + 实机资源替换 | P2 |
