...
class Various_CallUIBright : SubFragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        startTimePicker = TimePickerDialog(activity, { _, hourOfDay, minutes ->
            AppHelper.appPrefs?.edit()
                ?.putInt(key + "start_hour", hourOfDay)
                ?.putInt(key + "start_minute", minutes)
                ?.apply()
        ...
        endTimePicker = TimePickerDialog(activity, { _, hourOfDay, minutes ->
            AppHelper.appPrefs?.edit()
                ?.putInt(key + "end_hour", hourOfDay)
                ?.putInt(key + "end_minute", minutes)
                ?.apply()
    ...
    private fun updateStartTime(hr: Int, min: Int) {
        ...
        (findPreference<Preference>(key + "start") as? PreferenceEx)?.setCustomSummary(
            DateFormat.getTimeFormat(activity).format(time.time)
        )
    }
    ...
    private fun updateEndTime(hr: Int, min: Int) {
        ...
        time.set(Calendar.HOUR_OF_DAY, hr)
        time.set(Calendar.MINUTE, min)
        (findPreference<Preference>(key + "end") as? PreferenceEx)?.setCustomSummary(
            DateFormat.getTimeFormat(activity).format(time.time)
        )
    }
}
