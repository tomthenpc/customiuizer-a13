...
/**
 * Restricted decoder for historical Java-serialized A13 CustoMIUIzer backups.
 *
 * Does not use ObjectInputStream or ObjectInputFilter. Parses only the
 * ObjectOutputStream wire subset used by PreferenceFragmentBase's legacy
 * ObjectOutputStream(prefs.all) backups.
 */
object LegacyBackupDecoder {
    ...
    private class DecoderState(bytes: ByteArray) {
        ...
        fun readRootObject(): Any? = readContent()

        fun hasTrailingBytes(): Boolean = input.available() != 0
        ...
        private fun readObjectData(classDesc: ClassDesc): Any? {
            return when (classDesc.name) {
                ...
                "java.lang.Number" -> throw InvalidClassException(classDesc.name, "Unsupported legacy root object")
                else -> throw InvalidClassException(classDesc.name, "Legacy class not in allowlist")
            }
        }

        private data class MapBlockMetadata(val capacity: Int?, val loadFactor: Float?, val size: Int)
        private data class SetBlockMetadata(val capacity: Int?, val loadFactor: Float?, val size: Int)
        ...
        private fun readMapBlockHeader(): MapBlockMetadata {
            ...
            return when (blockLen) {
                4 -> MapBlockMetadata(null, null, input.readInt())
                8 -> MapBlockMetadata(input.readInt(), null, input.readInt())
                12 -> {
        ...
        private fun readHashSet(classDesc: ClassDesc): Set<Any?> {
            ...
            repeat(meta.size) { set.add(readContent()) }
            val endTc = input.readUnsignedByte()
            if (endTc != TC_ENDBLOCKDATA) {
        ...
        private fun readSetBlockHeader(): SetBlockMetadata {
            ...
            return when (blockLen) {
                4 -> SetBlockMetadata(null, null, input.readInt())
                12 -> {
                    val capacity = input.readInt()
                    val loadFactor = input.readFloat()
                    SetBlockMetadata(capacity, loadFactor, input.readInt())
                }
                8 -> SetBlockMetadata(input.readInt(), null, input.readInt())
        ...
        private fun readBoolean(classDesc: ClassDesc): Boolean =
            readPrimitiveWrapper(classDesc, 'Z') { input.readBoolean() } as Boolean

        private fun readInteger(classDesc: ClassDesc): Int =
            readPrimitiveWrapper(classDesc, 'I') { input.readInt() } as Int

        private fun readLong(classDesc: ClassDesc): Long =
            readPrimitiveWrapper(classDesc, 'J') { input.readLong() } as Long

        private fun readFloat(classDesc: ClassDesc): Float =
            readPrimitiveWrapper(classDesc, 'F') { input.readFloat() } as Float
        ...
        private fun consumeCustomData() {
            val tc = input.readUnsignedByte()
            if (tc == TC_BLOCKDATA) {
                skipBytes(input.readUnsignedByte())
            } else if (tc == TC_BLOCKDATALONG) {
                skipBytes(input.readInt())
        ...
        private fun validateClassDescriptor(classDesc: ClassDesc, schema: DescriptorSchema) {
            ...
            for (i in classDesc.fields.indices) {
                ...
                if (actual.type != expected.type || actual.name != expected.name || actual.className != expected.className) {
