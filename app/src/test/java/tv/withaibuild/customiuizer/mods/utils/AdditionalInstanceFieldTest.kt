...
/**
 * Pins the contract of the simulated instance fields on A13.
 *
 * These are read from hook bodies across SystemUI, Launcher and system_server, so the store has to
 * be correct under identity, mutation and concurrency all at once.
 */
class AdditionalInstanceFieldTest {

    /**
     * A class with value semantics whose hash depends on mutable state.
     *
     * This is the shape that broke the old `WeakHashMap` store: Launcher stashes `mLabelOrig` on a
     * `ShortcutInfo` and then rewrites `mLabel` on that same object.
     */
