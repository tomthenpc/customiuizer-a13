...
class B1MigrationInteropTest {
    ...
    fun categorySelectorCanBeInstantiatedAndExtendsSubFragment() {
        val cls = Class.forName("name.monwf.customiuizer.subs.CategorySelector")
    ...
    fun controlsCanBeInstantiatedAndExtendsSubFragment() {
        val cls = Class.forName("name.monwf.customiuizer.subs.Controls")
        assertTrue(SubFragment::class.java.isAssignableFrom(cls))
        assertNotNull(cls.getDeclaredConstructor().newInstance())
    }
    ...
    fun launcherCanBeInstantiatedAndExtendsSubFragment() {
        val cls = Class.forName("name.monwf.customiuizer.subs.Launcher")
        assertTrue(SubFragment::class.java.isAssignableFrom(cls))
        assertNotNull(cls.getDeclaredConstructor().newInstance())
    }
    ...
    fun colorSelectorCanBeInstantiatedAndExtendsSubFragment() {
        val cls = Class.forName("name.monwf.customiuizer.subs.ColorSelector")
        assertTrue(SubFragment::class.java.isAssignableFrom(cls))
        assertNotNull(cls.getDeclaredConstructor().newInstance())
    }
    ...
    fun mainFragmentB1FieldsRetainFqcnTypes() {
        val mainCls = Class.forName("name.monwf.customiuizer.MainFragment", false, loader)
        assertEquals("name.monwf.customiuizer.subs.CategorySelector", mainCls.getDeclaredField("catSelector").type.name)
        assertEquals("name.monwf.customiuizer.subs.Controls", mainCls.getDeclaredField("prefControls").type.name)
        assertEquals("name.monwf.customiuizer.subs.Launcher", mainCls.getDeclaredField("prefLauncher").type.name)
    }
