...
$ExpectedRepository = "github.com/tomthenpc/customiuizer-a13"
$ExpectedBranch = "devin/a13-rom-intelligence-audit"
...
if ([string]::IsNullOrWhiteSpace($Upstream)) {
    if ($Mode -eq "Audit" -or $ControlPlaneInstall) {
        Write-Warning "No upstream is configured. The Agent must set '$ExpectedUpstream' before Fast/Full/Final."
    }
    else {
...
if ($Mode -eq "Final") {
    ...
    $MainAhead = Get-GitText @("rev-list", "--count", "HEAD..origin/main") -AllowFailure
    Write-Host "Authorized branch synchronized: $LocalHead"
    if (-not [string]::IsNullOrWhiteSpace($MainAhead)) {
        Write-Host "Commits in origin/main not in HEAD: $MainAhead"
    }
}

Write-Host ""
Write-Host "A13 VERIFICATION PASSED"
Write-Host "Mode:   $Mode"
Write-Host "Branch: $Branch"
Write-Host "HEAD:   $Head"
exit 0
